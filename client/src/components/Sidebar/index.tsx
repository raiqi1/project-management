          ...
          {showTeamProjects &&
            ...
            projectTeams?.map((project: any) => (
              <SidebarLink
                ...
                icon={Briefcase}
                label={project.name}
                href={`/projects/${project.id}?team=true`}
              />
            ))}
...
const SidebarLink = ({ href, icon: Icon, label }: SidebarLinkProps) => {
  ...
  const isPathMatch = pathname === href || (href !== "/" && pathname.startsWith(href.split('?')[0]));
  ...
  const isActive = isPathMatch && ((href.includes('?team=true') && isTeamParam) || !href.includes('?team=true'));
